...
const fetchSharedWishlist = async () => {
  try {
    ...
    console.log('开始获取分享心愿单, 分享码:', route.params.code)
    await wishlistStore.fetchWishlistByShareCode(route.params.code)
    console.log('心愿单数据已获取:', wishlistStore.sharedWishlist)
    
    // 确保获取到了数据
    if (!wishlistStore.sharedWishlist) {
      throw new Error('获取心愿单数据失败')
    }
    
    // 检查API响应结构
    if (wishlistStore.sharedWishlist.data) {
      // 如果响应包含在data字段中，则使用data字段的内容
      wishlist.value = wishlistStore.sharedWishlist.data
      console.log('从data字段获取心愿单数据:', wishlist.value)
    } else {
      // 否则直接使用整个响应
      wishlist.value = wishlistStore.sharedWishlist
      console.log('直接使用响应数据:', wishlist.value)
    }
    ...
    if (wishlist.value) {
      ...
      throw new Error('获取心愿单数据失败，响应不包含有效数据')
    }
  } catch (err) {
    console.error('获取分享心愿单失败:', err)
    error.value = err.message || '加载心愿单失败'
  } finally {
    loading.value = false
  }
}
